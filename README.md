...
**⚠️ The judge is never shown the caller's quoted fee, and must not be again (fixed 2026-08-12).** It used to be passed in as context. On a real call the caller quoted **₹30L** and the judge returned a "published" fee of **exactly ₹30,00,000** — the number it had just been handed. An earlier call on the same institute had found ₹24.42L, so that was not the real published figure. The failure is silent and total: `fee_deviation` compares the two numbers, so a judge that echoes the quote makes them agree every time and **the check can never fire**. The judge's job is to read snippets, and it cannot parrot a figure it was never told. The comparison happens in `deriveFlags()` — in code, where it cannot be talked out of a verdict.

**Second lock: a published fee must be traceable to a snippet.** `feeAppearsIn()` parses every rupee figure the results actually state — `₹24,42,000`, `24.42 Lakhs`, `Rs 24.42L`, `2442000` are all the same number — and a fee the judge returns is discarded unless one of them matches within 1% (rounding to "24 lakh" is real and shouldn't cost a genuine figure). Same principle as the transcript quote check: a number on an officer's screen must trace back to something that actually said it. Discards are logged, not hidden. **Known limit:** the guard proves the number *appears*, not that it is a *total* — a per-year figure the judge wrongly promotes still passes, which is why the prompt rule about per-semester and per-year figures stays.
...
### 🔁 The second review round — four things from a real call (2026-08-12)

**1. It read our own lookup back as the caller's number.** The caller said the fee was ₹30L; `instituteVerify.js` found ₹24.42L published on the university's site; the agent said ₹24.42L back to them. Not a hallucination and not the weaker model — the prompt handed it `fee verified online: 2442000` sitting directly under `total fee: 3000000`, beneath a heading reading *"this is UPSY's own record of them, so you may use it directly"* and an instruction to confirm facts in passing. **Every `source: "api"` field is now withheld from the prompt**, derived from the schema rather than a name list, so the next lookup added is covered by default — including the two **CIBIL score** fields, unpopulated today and far worse to volunteer. Second cause: `isExcludedKey` ran only at depth 0, so it protected the root and let everything *inside* a branch through. It now applies at every depth. The caller's figure is the fact; ours is evidence, and `fee_deviation` on `/team` is where they are meant to meet.

**2. It asked questions the caller had already answered — and the first two fixes were both wrong.** Worth reading in full, because it is the most instructive failure in this repo.

The extractor is a model call running behind the conversation, so for a second or two the agenda still lists a field that was just answered, and the agent — doing as it is told — asks again.

*Attempt one:* extraction moved from every 3 caller turns to **every turn**, and `matchAgendaField()` was promoted to remember what had been asked. It was already word-matching every agent sentence to drive the call map, and the result was being thrown away, so this looked free.

*Attempt two* went further, on a reviewer's suggestion: since the matcher knows which field was just asked, plain code could read the answer and **file it immediately** — "my father earns 1.5 lakhs" → 150000, in the same tick, no model. That is `backend/fastAnswer.js`.

**Both rested on a premise that was false, and a real call proved it from both sides at once.** The agent said *"You'll need your PAN card and Aadhaar card. Do you have it?"*, the caller said *"Yes"*, and it was filed as **the applicant having a credit history** — one shared word, *card*, against "Has any card or loan already". And in the other direction, because the agent is instructed to ask in its own words, *"What does your father earn?"* and *"And his salary?"* matched **nothing at all**, so the question was never recorded and got asked again — the original complaint, untouched by the fix aimed at it.

**There is no threshold that fixes both.** Raise it and paraphrases are missed; lower it and wrong data is written. Adding a confidence score helped, and `keywords` on the fields people actually speak about (earn, salary, EMI, borrow, fee) helped more, but short questions legitimately share one word and no amount of tuning closes that gap. **`matchAgendaField` is bag-of-words overlap, and attribution is a language problem.** Its own comment always said it was cosmetic — *"a miss just leaves the spotlight where it was"* — and the mistake was promoting it out of that role without re-earning the trust.

**Where it landed:**

| Consumer | Bar | Why |
|---|---|---|
| Spotlight on the call map | any match | A wrongly lit dot costs nothing; a dark map costs the caller their sense of progress |
| Not re-asking a question | `isConfidentMatch` — 2+ tokens, clear winner | A wrong suppression means never asking at all |
| **Writing a value** | **off** (`VOICE_FAST_ANSWER=1`) | Attribution is not reliable enough to write a loan file |

**And the repeat is now fixed at the layer that already knows the answer: the prompt.** The agent has the whole conversation in front of it, and is told plainly that the agenda lags, that its own memory of the call wins, and not to re-ask anything it can see it already asked. Confirmed on a live call — no repeat, and coverage went **17 → 21 across two calls** without re-asking the twenty it already had.

`fastAnswer.js` stays in the tree because the mechanism is sound the moment attribution is; the reviewer's original suggestion — a **fast model classifying which field a question was about** — is what would make it safe, and that is not built.

**3. The document quiz was a no-op.** "Do you have your Aadhaar?" had nowhere to land: `aadhaarOnFile` and `panOnFile` are `source: "document"` and only an upload fills them. So the question spent the caller's time, taught us nothing, and made a helpful moment feel like an audit. The agent now **tells** them what they need and where to upload it. The three document questions that *do* have a field behind them and change the list — offer letter, ITR years, Form 16 years — are explicitly kept.

**4. It would not let a finished caller go.** `CLOSING_INTENT` matches unmistakable sign-offs only, in English and Hinglish, and the agent gets to answer and sign off before the socket closes. Verified on a real call: *"is it all done?"* → answered, then hung up by itself.

The phrases that end a call: **ok thanks · thank you · thanks a lot · nothing from my side · that's all · all done · no more questions · bye · ho gaya · bas itna hi · kuch nahi chahiye**.

Three things it must NOT do, all found by testing rather than reasoning:
- a bare **"no"** or **"nahi"** is an *answer*, not an ending
- **"that's all the money we have saved"** is a normal sentence about a down payment — it hung up on that in testing, so "that's all" must now END the utterance to count
- **"thanks, and what about the co-applicant?"** is someone thanking you and carrying on, which is why a bare thanks only closes when it is the **whole** utterance. A bare thanks was added later precisely because it is how most people actually finish, and "ok thanks" was leaving calls open

Saying "that's all" and then thinking of one more thing cancels the close, because that is exactly when people remember why they called. The goodbye gets `GOODBYE_GRACE_MS` to reach them, since synthesis finishing is not playback finishing.
...
- `backend/fastAnswer.js` — ⚠️ **built, then switched off (`VOICE_FAST_ANSWER=1` to enable).** Reads the answer to a question just asked, in plain code, with no model: "1.5 lakhs" → 150000 in the same tick. The parsing is careful and tested — one number per sentence only, so *"between 20 and 25 lakhs"* and *"I have 2 brothers and he earns 95000"* are both refused; never `text` fields; never overwrites. **What it cannot do is know which question it is answering**, because that comes from `matchAgendaField`, which is bag-of-words overlap. See the second review round above. Left in the tree because it becomes safe the moment attribution does.
...
- [x] **The applicant flow works on a phone** (2026-08-11): the document checklist became a drawer and Ask UPSY a bottom sheet, both reachable from the top bar and a floating button; every tap target inside them was 35px or smaller and is now 40–48px. Desktop re-measured pixel-identical. See the mobile items in the fine-tuning section
- [x] **A round trip between documents and voice** (2026-08-12): "Talk to agent" on every document page, "Upload documents" on `/upsy-voice-agent`. Same tab and same origin on purpose — the hop carries the lead id through `sessionStorage`, so the call lands **grounded in their own file rather than anonymous**, and carries the route back so they return to the document they left
- [x] **Callers can rate the call** (2026-08-12): `backend/reviews.js`, a sheet after the call and a third **Feedback** mode on `/team`. Only asks after 20+ seconds and 2+ turns, only asks once a week, and only a 1–2 star rating messages ops
- [x] **The agenda leads with what decides the loan** (2026-08-12): nine `essential` fields named first, so a call that ends at four minutes yields a decidable file instead of name, age and city
...
- [ ] **The institute/course catalogue** — the team is producing it. Turns the fee check into a lookup against our own data instead of reading it off search snippets, snaps speech-mangled institute names ("Symbiosis in Sudeep") to the real one, and lets the agent **confirm** course details rather than asking them cold

## Next (roadmap) — in likely priority order for a new session

**Where things stand (2026-08-12):** the full applicant + team flow is live on Render; the live-assist voice agent works in production; `/upsy-voice-agent` runs on our own voice stack, files each call into the branch schema, narrows the document list from what the call established, checks the named course against the web, and runs on Claude or the OpenAI side depending on one key in `.env`.
