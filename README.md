...
**2. It asked questions the caller had already answered.** The extractor is a model call running behind the conversation, so for a second or two the agenda still lists a field that was just answered — and the agent, doing as it is told, asks again. Two changes: extraction moved from **every 3 caller turns to every turn** (a fraction of a cent per call), and — the part that actually closes it — **the relay now remembers what it has asked**. `matchAgendaField()` was already word-matching every agent sentence to drive the call map, and the result was being thrown away; those hits now feed an `askedThisCall` set that removes the question from the agenda and names it under "ALREADY ASKED ON THIS CALL". No model call, no latency, current at the instant a question leaves the agent's mouth. It suppresses the *ask*, never the answer.
...
**4. It would not let a finished caller go.** `CLOSING_INTENT` matches unmistakable sign-offs only, in English and Hinglish, and the agent gets to answer and sign off before the socket closes. Two things it must not do, both real: a bare **"no"** or **"nahi"** is an *answer*, not an ending; and **"that's all the money we have saved"** is a normal sentence about a down payment, so that phrase must end the utterance to count — it hung up on that sentence in testing before the pattern was tightened. Saying "that's all" and then thinking of one more thing cancels the close, because that is exactly when people remember why they called. The goodbye gets `GOODBYE_GRACE_MS` to reach them, since synthesis finishing is not playback finishing.
