...
**Known gaps, honestly:**
- **Wired onto Render, not yet proven there (2026-07-31).** `AGENTCALL_API_KEY` is now declared in `render.yaml` and filled in on the dashboard (bridge.js already reads this env var directly, same as it reads `~/.agentcall/config.json` locally — no code change needed); the redeployed instance is confirmed up and signing in leads correctly, but an actual live-assist call joining a meeting *from the deployed instance* hasn't been end-to-end tested yet — only the local instance has a confirmed live join.
...
- **Re-deployed 2026-07-31** (manual deploy, after `AGENTCALL_API_KEY` was added to the Blueprint and filled in on the dashboard): confirmed the redeployed instance is up (`/login` returns 200) and `/api/session/start` correctly signs in a demo lead (`LD-1001` / Aarav Sharma) on the fresh container. **Not yet confirmed**: an actual live-assist call joining a meeting on the *deployed* instance specifically — bridge.js's own AgentCall connection from Render hasn't been end-to-end tested yet, only the general app + the env var wiring.
...
- [x] ~~Wire AgentCall onto the deployed Render instance~~ — done 2026-07-31: `AGENTCALL_API_KEY` declared in `render.yaml`, filled in on the dashboard, redeployed and confirmed up. Still open: an actual live-assist call joining a meeting *from the deployed instance* hasn't been tested yet — do that next.
...
- [ ] **Explore going 100% own-stack instead of AgentCall** (team direction, 2026-07-31) — the team already has Deepgram and Sarvam API keys (per the secrets note in Code map) plus other providers, and wants to look at building the live-call layer in-house rather than depending on AgentCall long-term. **Important scope caveat, from the dependency breakdown above**: Deepgram (speech-to-text) and Sarvam (Indian-language STT/TTS) would replace the *voice* commodity layer, which AgentCall's own pricing already treats as swappable ("bring your own transcription/TTS"). They do **not** replace the part that's actually hard and actually AgentCall's real product: getting a bot into a live Google Meet/Zoom/Teams call at all (browser automation per platform, waiting-room handling, pulling real meeting audio out, injecting synthesized audio back in, screenshotting the shared screen). Going "100% our own" would mean building or licensing *that* piece too, not just swapping STT/TTS providers — worth pricing out honestly before committing, since it's the same weeks-to-months-of-engineering estimate as before, just now with a team mandate behind it rather than a hypothetical.
