...
**Known gaps, honestly:**
- **Local-only so far.** The AgentCall API key lives in `~/.agentcall/config.json` on the dev machine, not as a Render env var — none of this works on the deployed Render instance yet without that setup.
...
- [ ] Wire AgentCall onto the deployed Render instance (env var for the API key instead of the local `~/.agentcall/config.json` file) — currently local-only.
